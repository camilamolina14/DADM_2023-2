package com.molinacamila.reto03
...
class MainActivity : AppCompatActivity() {
    ...
    // Handle the click event on yhe board
    ...
    // Update the game board after every move
    fun playNow(buttonSelected:Button , currCell:Int)
    {
        if(activeUser == 1)
        {
            buttonSelected.text = "X"
            buttonSelected.setTextColor(Color.parseColor("#E91E63"))
            ...
            if(checkWinner == 1){
                ...
                Handler().postDelayed(Runnable { androidPlayer() } , 500)
        }
        else
        {
            buttonSelected.text = "O"
            buttonSelected.setTextColor(Color.parseColor("#02A602"))
            activeUser = 1
    ...
    fun checkWinner(): Int {
        if ((player1.contains(1) && player1.contains(2) && player1.contains(3)) || (player1.contains(
            ...
            val build = AlertDialog.Builder(this)
            build.setTitle("Tie! No one has won")
            build.setMessage("Do you want to play again?")
            build.setPositiveButton("Yes") { dialog, which ->
                reset()
            }
    ...
    fun reset()
    {
        ...
        for(i in 1..9)
        {
            ...
            buttonselected.text = ""
            textView.text = "Human : $player1Count"
            textView2.text = "Android : $player2Count"
        }
    }

    // Automatically make a move at a random position
    fun androidPlayer()
    {
        val rnd = (1..9).random()
    ...
    // Disable all the button onthe board for a while
    fun disableReset() {
