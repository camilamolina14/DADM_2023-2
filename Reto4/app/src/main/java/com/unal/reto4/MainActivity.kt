...
class MainActivity : AppCompatActivity(){
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId){
            R.id.salir ->{
                val salir = Dialogues.newInstance(Dialogues.DIALOG_TYPE_ONE)
                salir.show(supportFragmentManager,"Go out")
                true
            }
            R.id.new_game->{
                val nuevo = Dialogues.newInstance(Dialogues.DIALOG_TYPE_DEFAULT)
                nuevo.show(supportFragmentManager,"New game?")
                true
            }
            R.id.nivel->{
                val nivel = Dialogues.newInstance(Dialogues.DIALOG_TYPE_TWO)
                nivel.show(supportFragmentManager,"Level")
                true
            }
